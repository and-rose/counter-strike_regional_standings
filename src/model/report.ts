...
function formatLine(line: string, newline = false) {
  let output = line + "<br />\n";
  if (newline) output = line + "\n";
  return output;
}
...
function generateOutput(teams: Team[], strDate: string) {
  ...
  if (dayOfMonth < 8) {
    if (!fs.existsSync(invitationFolder + `${summaryFolder}${fileDate}/`))
      fs.mkdirSync(invitationFolder + `${summaryFolder}${fileDate}/`, {
        recursive: true,
      });

    fs.writeFileSync(
      `${invitationFolder}standings_global_${fileDate}${format}`,
      displayRankings(teams, [0, 1, 2], strDate),
    );
    fs.writeFileSync(
      `${invitationFolder}standings_europe_${fileDate}${format}`,
      displayRankings(teams, [0], strDate),
    );
    fs.writeFileSync(
      `${invitationFolder}standings_americas_${fileDate}${format}`,
      displayRankings(teams, [1], strDate),
    );
    fs.writeFileSync(
      `${invitationFolder}standings_asia_${fileDate}${format}`,
      displayRankings(teams, [2], strDate),
    );
  }

  if (!fs.existsSync(liveFolder + `${summaryFolder}${fileDate}/`))
    fs.mkdirSync(liveFolder + `${summaryFolder}${fileDate}/`, {
      recursive: true,
    });

  fs.writeFileSync(
    `${liveFolder}standings_global_${fileDate}${format}`,
    displayRankings(teams, [0, 1, 2], strDate),
  );
  fs.writeFileSync(
    `${liveFolder}standings_europe_${fileDate}${format}`,
    displayRankings(teams, [0], strDate),
  );
  fs.writeFileSync(
    `${liveFolder}standings_americas_${fileDate}${format}`,
    displayRankings(teams, [1], strDate),
  );
  fs.writeFileSync(
    `${liveFolder}standings_asia_${fileDate}${format}`,
    displayRankings(teams, [2], strDate),
  );
  ...
  teams.forEach((t) => {
    if (t.globalRank > 0) {
      ...
      fs.writeFileSync(
        `${liveFolder}${filename}`,
        displayTeamRankingSummary(t, teams, strDate),
      );

      if (dayOfMonth < 8) {
        fs.writeFileSync(
          `${invitationFolder}${filename}`,
          displayTeamRankingSummary(t, teams, strDate),
        );
      }
    }
...
function sortCaseInsensitive(list: string[]) {
...
function sanitize(text: string, replacement = "-") {
...
function displayRankings(teams: Team[], regions: number[], strDate: string) {
  ...
  teams.forEach((t) => {
    if (t.globalRank > 0 && regions.some((r) => t.region[r] === 1)) {
      ...
      table.addElem(
        `[details](${summaryFolder}${fileDate}/${paddedRank}--${sanitize(t.name)}--${sanitizeRoster(t.players)}${format})`,
      );
      table.commitRow();
    }
  });
...
function displayTeamRankingSummary(team: Team, teams: Team[], strDate: string) {
  ...
  output += formatLine(
    `Global Rank: [${team.globalRank}](../../standings_global_${fileDate}.md)`,
  );
  output += formatLine("");
  ...
  team.region.forEach((r, idx) => {
    if (r === 1) {
      ...
      output += formatLine(
        `Region: [${region}]( ../../${regionFile}_${fileDate}.md)`,
      );
      output += formatLine(
        `Regional Rank: [${team.regionalRank[idx]}]( ../../${regionFile}_${fileDate}.md)`,
      );
      output += formatLine("");
    }
  });
  ...
  output += formatLine("");
  output += formatLine(
    `Final Rank Value (${team.rankValue.toFixed(1)}) = Starting Rank Value (${team.rankValueSeed.toFixed(1)}) + Head To Head Adjustments (${(team.rankValue - team.rankValueSeed).toFixed(1)})`,
  );
  output += formatLine("", true);
  output += formatLine("#### Starting Rank Value");
  output += formatLine(
    `To figure out a rosters's Starting Rank Value, first take the average of these four factors:`,
  );
  output += formatLine(
    `- Bounty Offered: ${team.modifiers.bountyOffered.toFixed(3)}[<sup>1</sup>](#table2)`,
    true,
  );
  output += formatLine(
    `- Bounty Collected: ${team.modifiers.bountyCollected.toFixed(3)}[<sup>2</sup>](#table1)`,
    true,
  );
  output += formatLine(
    `- Opponent Network: ${team.modifiers.opponentNetwork.toFixed(3)}[<sup>2</sup>](#table1)`,
    true,
  );
  output += formatLine(
    `- LAN Wins: ${team.modifiers.lanFactor.toFixed(3)}[<sup>2</sup>](#table1)`,
    true,
  );
  output += formatLine("", true);
  output += formatLine(
    `The average of these factors is ${team.seedValue.toFixed(3)}`,
  );
  output += formatLine("");
  output += formatLine(
    `Next, take the maximum and minimum average across all teams and compute the following:`,
  );
  output += formatLine(
    `- 400 + ( ( Roster_Average - Min_Average ) / ( Max_Average - Min_Average ) ) * 1600 = ${team.rankValueSeed.toFixed(1)}`,
    true,
  );
  output += formatLine(
    `- 400 + ( ( ${team.seedValue.toFixed(3)} - ${minSeedValue.toFixed(3)} ) / ( ${maxSeedValue.toFixed(3)} - ${minSeedValue.toFixed(3)} ) ) * 1600 = ${team.rankValueSeed.toFixed(1)}`,
    true,
  );
  output += formatLine("", true);
  output += formatLine("", true);
  ...
  output += formatLine(
    `Below you can see a table of all of the matches that contributed to this roster's Final Rank Value.`,
  );
  ...
  team.teamMatches.forEach((tm, idx) => {
    ...
    let bountyEl = team.bounties.filter((el) => {
      return el.id === umid;
    });
    if (bountyEl.length > 0) {
      bounty =
        bountyEl[0].base.toFixed(3).toString() +
        " (" +
        bountyEl[0].val.toFixed(3).toString() +
        ")";
      eventWeight = bountyEl[0].context.toFixed(3).toString();
    }

    let netEl = team.network.filter((el) => {
      return el.id === umid;
    });
    if (netEl.length > 0) {
      net =
        netEl[0].base.toFixed(3).toString() +
        " (" +
        netEl[0].val.toFixed(3).toString() +
        ")";
      eventWeight = netEl[0].context.toFixed(3).toString();
    }

    let lanWinEl = team.lanWins.filter((el) => {
      return el.id === umid;
    });
    if (lanWinEl.length > 0) {
      lanWin =
        lanWinEl[0].base.toString() +
        " (" +
        lanWinEl[0].val.toFixed(3).toString() +
        ")";
    }
  ...
  output += formatLine("", true);
  output += formatLine(
    `- First, take the sum of their top 10 scaled winnings (${USDollar.format(Number(team.scaledWinnings.toFixed(2)))})`,
    true,
  );
  output += formatLine(
    `- Divide that value by the 5th highest value among all rosters (${USDollar.format(
      nthHighest(
  ...
  output += formatLine(
    `- The final value (${team.bountyOffered.toFixed(2)}) is scaled by the curve function.[<sup>3</sup>](#curveFunction)`,
    true,
  );
  ...
  output += formatLine("", true);
  output += formatLine(
    `<span id="curveFunction"></span>_The Curve Function: 1 / ( 1 + abs( log10( x ) ) )_`,
  );
  output += formatLine("", true);
  output += formatLine("---", true);
  output += formatLine(
